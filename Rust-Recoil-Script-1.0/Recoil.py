...
def run():
    ...
    while active: #Main loop
        ...
        if not paused:
            ...
            if win32api.GetKeyState(0x06) < 0: #xMB1
                ...
                engine.runAndWait()
            if win32api.GetKeyState(0x43) < 0: #c
                active_weapon = weapon_change(-1)
                engine.say(all_weapons[active_weapon])
                engine.runAndWait()
            if win32api.GetKeyState(0x05) < 0: #xMB2
                scope_change()
                engine.say(all_scopes[active_scope])
                engine.runAndWait()
        #Doesnt Matter if Paused
        if win32api.GetKeyState(0x26) < 0: #ScrLk
            get_sense()
            engine.say("Updated")
            engine.runAndWait()
        if win32api.GetKeyState(0x56) < 0: #v
            paused = not paused
            if paused:
                engine.say("Paused")
                engine.runAndWait()
                ...
                engine.say("Unpaused")
                engine.runAndWait()
        if win32api.GetKeyState(0x28) < 0: #dwnArrow 
                engine.say("Exiting")
                engine.runAndWait()
                proc.terminate()
                active = False